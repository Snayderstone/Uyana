...
<script lang="ts">
	...
	$: percent = total !== null && total > 0 ? Math.max(0, Math.min(100, (+value / total) * 100)) : 0;
	...
	// Textos mostrados
	$: displayInsideText = insideText || `${Math.round(animatedValue)}%`;
	$: displayValue = typeof value === 'number' ? value.toLocaleString() : value;
	$: displayTotal = total !== null ? total.toLocaleString() : '';
	// Cuando cambia percent (el real), lanzamos la animación:
	...
	function animateToPercent(target: number) {
		...
		function step(now: number) {
			...
			animatedPercent = start + (end - start) * progress;
...
<style lang="scss">
	.circular-status {
		...
		background: color-mix(
			in srgb,
			var(--bg),
			transparent calc((1 - var(--bg-opacity, 0.85)) * 100%)
		);
