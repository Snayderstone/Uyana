...
<script lang="ts">
  ...
  export let popupFormatter: (props: any, id: string, value: number | null) => string = (p, id, value) => {
    const vtxt = value == null ? '–' : String(value);
    const rows = Object.entries(p || {}).map(([k, v]) => `<div><b>${k}:</b> ${v}</div>`).join('');
  ...
  function bindEvents(feature: any, layer: any) {
    ...
    layer.on('mouseover', (e: any) => {
      ...
      if (path) path.style.filter = 'drop-shadow(0 0 6px rgba(110,41,231,.55)) drop-shadow(0 0 14px rgba(110,41,231,.28))';
