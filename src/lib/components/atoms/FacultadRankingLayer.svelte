<script lang="ts">
  ...
  function iconHTML(rank: number): string {
    ...
    if (color) {
      ...
      return `
        <svg class="laurel-badge" viewBox="0 0 17714 15065" width="28" height="28" aria-hidden="true">
  ...
  function drawMarkers() {
    ...
    ordenadas.forEach(({ rank, center }) => {
      const icon = L!.divIcon({
        ...
        html: iconHTML(rank),//aquí metemos el laurel para top-3
        iconSize: [28, 28],
        iconAnchor: [14, 14]
      });
...
<style>
  ...
  :global(.faculty-rank-icon) { pointer-events: none !important; }
