<script lang="ts">
	...
	function drawMarkers() {
		...
		ordenadas.forEach(({ rank, center }) => {
			...
			if (zoom < SHOW_TOP_ZOOM) return;
			if (zoom < SHOW_ALL_ZOOM && rank > 3) return;
			const size = markerSize(rank);
			...
			const marker = L!
				.marker(center as [number, number], {
					icon,
					interactive: false, //sigue sin hover ni click
					keyboard: false
				})
				.addTo(map);
...
<!--Evita el warning "No scopable elements found" -->
<div style="display:none;" />

<style>
	/* Mantienes el icono no interactivo */
	:global(.faculty-rank-icon) {
		pointer-events: none !important;
	}

	/* Tu estilo original para los que NO son top-3 */
	...
	/* El <svg> del top-3 no tiene fondo: solo trazo (stroke) y número centrado con halo */
	:global(.faculty-rank-icon .laurel-badge) {
