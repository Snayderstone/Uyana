...
import { AdminProjectsRepository } from '$lib/db/admin/projects/projects.repository';
import {
...
export const AdminProjectsService = {
	...
	/**
	 * Obtener TODOS los proyectos sin paginación (para exportación)
	 */
	async getAllProjects(): Promise<ProyectoResponseDTO[]> {
		const { data } = await AdminProjectsRepository.listProjects(1, 999999, {});
		...
		// Construir DTOs completos con todas las relaciones
		const proyectosPromises = data.map(async (proyecto) => {
			const estado = estadosMap.get(proyecto.estado_id);

			// Obtener todas las relaciones (IDs)
			const [tiposIds, areasIds, lineasIds, institucionesIds, fuentesIds] = await Promise.all([
				AdminProjectsRepository.getProjectTypes(proyecto.id),
				AdminProjectsRepository.getProjectAreas(proyecto.id),
				AdminProjectsRepository.getProjectLines(proyecto.id),
				AdminProjectsRepository.getProjectInstitutions(proyecto.id),
				AdminProjectsRepository.getProjectFunding(proyecto.id)
			]);

			// Obtener los objetos completos de cada relación
			const [tipos, areas, lineas, instituciones, fuentes] = await Promise.all([
				Promise.all(tiposIds.map((id) => AdminTiposRepository.getById(id))),
				Promise.all(areasIds.map((id) => AdminAreasRepository.getById(id))),
				Promise.all(lineasIds.map((id) => AdminLineasRepository.getById(id))),
				Promise.all(institucionesIds.map((id) => AdminInstitucionesRepository.getById(id))),
		...
		return await Promise.all(proyectosPromises);
	},
