/**
 * Herramienta MCP para consultar estadísticas de UYANA
 ...
 */
const estadisticasUyanaSchema = z.object({
	consulta: z
		.string()
...
export type EstadisticasUyanaArgs = z.infer<typeof estadisticasUyanaSchema>;
...
function formatearResumenEjecutivo(resumen: any): string {
	return `📊 **Resumen Ejecutivo de UYANA**
 ...
 */
async function manejarEstadisticasUyana(args: any): Promise<McpResponse> {
	try {
				...
				content: [
					{
						type: 'text',
						text:
							'👋 ¡Hola! Soy el módulo de estadísticas de UYANA. Para obtener información, puedes preguntarme sobre:\n\n' +
		...
		console.error('Error en estadisticasUyanaTool:', error);
		return {
			content: [
				{
					type: 'text',
 ...
 */
export const estadisticasUyanaTool: McpTool<EstadisticasUyanaArgs> = {
	name: 'estadisticas-uyana',
	title: 'Estadísticas de UYANA',
	description:
		'Proporciona información estadística sobre proyectos de investigación, participantes y facultades',
	category: 'information',
	schema: estadisticasUyanaSchema,
	handler: manejarEstadisticasUyana,
	metadata: {
		version: '1.0.0',
		author: 'UYANA Team',
		supportedQueries: [
		...
		helpInfo: {
			title: '📊 Estadísticas de UYANA',
			description:
				'Esta herramienta te permite consultar información detallada sobre los proyectos de investigación de la Universidad Central del Ecuador (UCE) gestionados en la plataforma UYANA.',
			howToUse: [
